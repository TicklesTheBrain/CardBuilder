...
@export var cardArea: CollisionShape2D
...
func scuttleCards():
	...
	var centerPos = cardArea.position
	var areaWidth = cardArea.shape.get_rect().size.x
	...
	if cards.size() == 1:
		...
		startingPoint = centerPos - Vector2(areaWidth/2,0)
	else:
	...
	if not startingPoint:
		startingPoint = centerPos - Vector2(totalCardWidth/2, 0)
